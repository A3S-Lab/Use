use a3s_use_core::{PluginManagerToolset, PLUGIN_MANAGER_TOOLSET_SCHEMA_V3};

const TOOLSET: &[u8] = include_bytes!("../fixtures/plugins/manager-toolset-v3.json");
...
fn current_manager_toolset_fixture_is_canonical_and_frozen() {
    ...
    assert_eq!(toolset, PluginManagerToolset::v3());
    assert_eq!(toolset.schema, PLUGIN_MANAGER_TOOLSET_SCHEMA_V3);
    assert_eq!(
    ...
    let mut retired = serde_json::to_value(PluginManagerToolset::v3()).unwrap();
    retired["schema"] = serde_json::json!("a3s.use.plugin-manager-tools.v2");
    let error =
