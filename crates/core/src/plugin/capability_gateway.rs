...
fn decode_capability_descriptor(value: Value) -> Result<CapabilityDescriptor, String> {
    ...
    let capability = match wire.kind.as_str() {
        "tool" => {
            ...
            {
                return Err("A Tool descriptor cannot contain MCP server fields.".to_owned());
            }
        ...
        "mcp-server" => {
            ...
            {
                return Err("An MCP Server descriptor cannot contain Tool fields.".to_owned());
            }
...
impl CapabilityDescriptor {
    ...
    pub fn tool_name(&self) -> Option<&str> {
        match &self.capability {
            ...
            CapabilityDescriptorKind::McpServer { .. } => None,
        }
    }

    pub fn mcp_server_name(&self) -> Option<&str> {
        match &self.capability {
            CapabilityDescriptorKind::Tool { .. } => None,
...
) -> (String, PluginSurfaceKind, String, u64) {
    (
