...
```bash
a3s use browser --webgpu open https://my-webgpu-app.example.com
# wait for the app to render (see "Timing" below)
a3s use browser screenshot app.png
```

`--webgpu` (or `A3S_USE_BROWSER_WEBGPU=1`, or `"webgpu": true` in a3s use browser.json) applies a launch preset:
...
The Windows/Linux screenshot gap is an upstream headless-Chrome limitation: WebGPU canvas *presentation* never reaches the headless compositor, even though rendering itself works (verified by pixel readback). It is not an a3s use browser or flag problem — no known flag combination fixes it. Rendering, `eval`-based pixel readbacks, and compute all work headless everywhere.

On Linux, `--headed` is all you need even on displayless servers and containers: when no `DISPLAY` is set and Xvfb is installed (`apt-get install -y xvfb`), a3s use browser starts a private virtual display for the browser and tears it down with it. Set `A3S_USE_BROWSER_NO_XVFB=1` to opt out.

```bash
a3s use browser --webgpu --headed open https://my-webgpu-app.example.com
a3s use browser screenshot app.png   # real WebGPU pixels, no display hardware
```
...
Add `--headed` (`a3s use browser doctor --webgpu --headed`) to validate the capture path itself — on displayless Linux the probe starts its own Xvfb, so both checks should pass.
...
Container recipe (Debian/Ubuntu base; xvfb needed only for the screenshot path). Verified with both Chrome for Testing and Debian's `chromium` package (set `A3S_USE_BROWSER_EXECUTABLE_PATH=/usr/bin/chromium` for the latter — useful on ARM64, where Chrome for Testing has no Linux builds):

```dockerfile
FROM debian:bookworm-slim
ARG A3S_USE_VERSION=0.1.0
RUN apt-get update && apt-get install -y \
    ca-certificates curl libvulkan1 mesa-vulkan-drivers xvfb xauth \
    && rm -rf /var/lib/apt/lists/*
RUN set -eux; \
    archive="a3s-use-${A3S_USE_VERSION}-linux-x86_64.tar.gz"; \
    base="https://github.com/A3S-Lab/Use/releases/download/v${A3S_USE_VERSION}"; \
    mkdir -p /usr/local/lib/a3s-use; \
    curl --fail --location --proto '=https' -o "/tmp/${archive}" "${base}/${archive}"; \
    curl --fail --location --proto '=https' -o /tmp/checksums.txt "${base}/checksums.txt"; \
    cd /tmp; \
    grep "  ${archive}$" checksums.txt | sha256sum --check -; \
    tar -xzf "${archive}" -C /usr/local/lib/a3s-use; \
    ln -s /usr/local/lib/a3s-use/a3s-use /usr/local/bin/a3s-use; \
    ln -s /usr/local/lib/a3s-use/a3s-use-browser-driver /usr/local/bin/a3s-use-browser-driver; \
    rm -f "/tmp/${archive}" /tmp/checksums.txt
RUN a3s-use browser install --with-deps
```
...
```bash
a3s use browser --webgpu --args "--use-vulkan=native,--use-webgpu-adapter=default" open ...
```
...
```bash
a3s use browser wait --fn "window.__appReady === true"
# or generically: give the render loop a frame or two
a3s use browser eval "new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)))"
a3s use browser screenshot app.png
```
...
```bash
a3s use browser eval "document.querySelector('canvas').getContext('webgpu') ? 'webgpu' : 'webgl-fallback'"
```
