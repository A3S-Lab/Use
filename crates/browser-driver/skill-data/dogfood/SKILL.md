...
Always use `a3s use browser` directly. Do not prefix it with a JavaScript
package runner; A3S installs and owns the Browser component lifecycle.
...
```bash
a3s use browser --session {SESSION} open {TARGET_URL}
a3s use browser --session {SESSION} wait --load networkidle
```
...
```bash
a3s use browser --session {SESSION} snapshot -i
# Identify login form refs, fill credentials
a3s use browser --session {SESSION} fill @e1 "{EMAIL}"
a3s use browser --session {SESSION} fill @e2 "{PASSWORD}"
a3s use browser --session {SESSION} click @e3
a3s use browser --session {SESSION} wait --load networkidle
```
...
```bash
a3s use browser --session {SESSION} state save {OUTPUT_DIR}/auth-state.json
```
...
```bash
a3s use browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/initial.png
a3s use browser --session {SESSION} snapshot -i
```
...
```bash
a3s use browser --session {SESSION} snapshot -i
a3s use browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/{page-name}.png
a3s use browser --session {SESSION} errors
a3s use browser --session {SESSION} console
```
...
```bash
a3s use browser --session {SESSION} record start {OUTPUT_DIR}/videos/issue-{NNN}-repro.webm
```
...
```bash
a3s use browser --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-1.png
sleep 1
# Perform action (click, fill, etc.)
sleep 1
a3s use browser --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-2.png
sleep 1
...
```bash
sleep 2
a3s use browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}-result.png
```

4. **Stop the video:**

```bash
a3s use browser --session {SESSION} record stop
```
...
```bash
a3s use browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}.png
```
...
```bash
a3s use browser --session {SESSION} close
```
...
- **Be efficient with commands.** Batch multiple `a3s use browser` commands in a single shell call when they are independent (e.g., `a3s use browser ... screenshot ... && a3s use browser ... console`). Use `a3s use browser --session {SESSION} scroll down 300` for scrolling -- do not use `key` or `evaluate` to scroll.
