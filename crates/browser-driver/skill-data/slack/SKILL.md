...
a3s use browser open https://app.slack.com
```
...
a3s use browser click @e21  # Ref for "More unreads" button
a3s use browser screenshot slack-unreads.png
```
...
a3s use browser click @e14  # Activity tab (ref may vary)
a3s use browser wait 1000
a3s use browser screenshot activity-unreads.png

# Or check DMs tab
a3s use browser click @e13  # DMs tab
a3s use browser screenshot dms.png

# Or expand "More unreads" in sidebar
a3s use browser click @e21  # More unreads button
a3s use browser wait 500
a3s use browser screenshot expanded-unreads.png
```
...
a3s use browser click @e94  # Example: engineering channel ref
a3s use browser wait --load networkidle
a3s use browser screenshot channel.png
```
...
# Use Slack search
a3s use browser snapshot -i
a3s use browser click @e5  # Search button (typical ref)
a3s use browser fill @e_search "keyword"
...
a3s use browser snapshot --json > slack-snapshot.json
...
a3s use browser snapshot -i
a3s use browser screenshot channel-details.png

# Scroll through messages
a3s use browser scroll down 500
a3s use browser screenshot channel-messages.png
```
...
a3s use browser screenshot --annotate slack-state.png

# Take full-page screenshot
a3s use browser screenshot --full slack-full.png
...
a3s use browser get text @e_message_ref
```
...
a3s use browser snapshot --json > output.json
...
a3s use browser snapshot -i | grep -c "treeitem"
# Each treeitem with a channel name in the unreads section is one unread
```

## Best Practices

- **Connect to existing sessions**: Use `a3s use browser connect 9222` if Slack is already open. This is faster than opening a new browser.
...
- **Scroll in sidebar**: Use `a3s use browser scroll down 300 --selector ".p-sidebar"` to scroll within the Slack sidebar if channel list is long.
...
a3s use browser get url
a3s use browser get title
a3s use browser screenshot page-state.png
```
...
a3s use browser snapshot -i > snapshot.txt

# Check Activity tab for unreads
a3s use browser click @e14  # Activity tab
a3s use browser wait 1000
a3s use browser screenshot activity.png
ACTIVITY_RESULT=$(a3s use browser get text @e_main_area)
...
a3s use browser click @e21  # More unreads button
a3s use browser wait 500
a3s use browser snapshot -i > unreads-expanded.txt
a3s use browser screenshot unreads.png
