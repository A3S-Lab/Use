---
name: electron
description: Automate Electron desktop apps (VS Code, Slack, Discord, Figma, Notion, Spotify, etc.) using a3s use browser via Chrome DevTools Protocol. Use when the user needs to interact with an Electron app, automate a desktop app, connect to a running app, control a native app, or test an Electron application. Triggers include "automate Slack app", "control VS Code", "interact with Discord app", "test this Electron app", "connect to desktop app", or any task requiring automation of a native Electron application.
allowed-tools: Bash(a3s:*)
---

# Electron App Automation

Automate any Electron desktop app using a3s use browser. Electron apps are built on Chromium and expose a Chrome DevTools Protocol (CDP) port that a3s use browser can connect to, enabling the same snapshot-interact workflow used for web pages.

## Core Workflow

1. **Launch** the Electron app with remote debugging enabled
2. **Connect** a3s use browser to the CDP port
...
a3s use browser --cdp 9222 snapshot -i

# Auto-discover a running Chromium-based app
a3s use browser --auto-connect snapshot -i
```
...
a3s use browser tab 2

# Switch by URL pattern
a3s use browser tab --url "*settings*"
```
...
# Switch to a webview
a3s use browser tab 1

# Interact with the webview normally
a3s use browser snapshot -i
a3s use browser click @e3
a3s use browser screenshot webview.png
```
...
a3s use browser click @e10  # Navigate to a section
a3s use browser snapshot -i  # Re-snapshot after navigation
```
...
```bash
a3s use browser connect 9222
a3s use browser screenshot app-state.png
a3s use browser screenshot --full full-app.png
a3s use browser screenshot --annotate annotated-app.png
```
...
a3s use browser snapshot --json > app-state.json
```
...
a3s use browser connect 9222
a3s use browser snapshot -i
a3s use browser fill @e3 "search query"
...
```bash
# Connect to Slack
a3s use browser --session slack connect 9222

# Connect to VS Code
a3s use browser --session vscode connect 9223

# Interact with each independently
a3s use browser --session slack snapshot -i
a3s use browser --session vscode snapshot -i
```
...
```bash
a3s use browser connect 9222
a3s use browser --color-scheme dark snapshot -i
```

Or set it globally:

```bash
A3S_USE_BROWSER_COLOR_SCHEME=dark a3s use browser connect 9222
```
...
- The app may use multiple webviews. Use `a3s use browser tab` to list targets and switch to the right one

### Cannot type in input fields

- Try `a3s use browser keyboard type "text"` to type at the current focus without a selector
- Some Electron apps use custom input components; use `a3s use browser keyboard inserttext "text"` to bypass key events
...
If an app is built with Electron, it supports `--remote-debugging-port` and can be automated with a3s use browser.
