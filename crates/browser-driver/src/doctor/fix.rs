...
fn attempt_chrome_install() -> bool {
    // The compatibility install command delegates to the A3S component
    // lifecycle and returns its process status, so run it out of process.
...
mod tests {
    ...
    fn test_run_fixes_generates_missing_encryption_key() {
        // Override HOME so get_state_dir() resolves under a temporary A3S
        // state root and cannot touch the caller's files.
        ...
        .with_fix("export A3S_USE_BROWSER_ENCRYPTION_KEY=...")];
        ...
        assert!(
            tmp.path()
                .join(".local/state/a3s/use/browser/.encryption-key")
                .exists(),
            "key file should exist in the A3S Use Browser state root"
        );
    }
}
