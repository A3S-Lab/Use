...
///
/// - `skills/` — discovery stubs for Skill package installers. Carry
...
///
/// Both are shipped in the A3S Use release and searched by `discover_skills`.
const SKILL_DIRS: &[&str] = &["skills", "skill-data"];

/// Locate the package root that contains the skill directories.
///
/// Resolution order:
/// 1. A3S_USE_BROWSER_SKILLS_DIR env var (points directly at a single directory)
/// 2. ../ relative to the executable (standard `bin/` + `share/` installs)
...
fn find_package_root() -> Option<PathBuf> {
    if let Ok(exe) = env::current_exe() {
        ...
        if let Some(parent) = exe.parent() {
            // Standard install layout: bin/a3s-use-browser-driver -> ../
...
pub(crate) fn find_skills_dirs() -> Vec<PathBuf> {
    // An explicit A3S path wins. The legacy name remains a lower-precedence
    // compatibility alias for existing agent-browser integrations.
    for name in ["A3S_USE_BROWSER_SKILLS_DIR", "AGENT_BROWSER_SKILLS_DIR"] {
        if let Ok(dir) = env::var(name) {
    ...
    if targets.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "No skill name provided. Usage: a3s use browser skills get <name>",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} No skill name provided. Usage: a3s use browser skills get <name>",
    ...
    if skills_dirs.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "Skills directory not found. Set A3S_USE_BROWSER_SKILLS_DIR or run 'a3s install use'.",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} Skills directory not found. Set A3S_USE_BROWSER_SKILLS_DIR or run 'a3s install use'.",
                color::error_indicator()
            );
        }
        exit(1);
