...
//! A3S Use Browser keeps ownership of browser automation, policy checks, and
...
fn config_path_for_scope(scope: &PluginConfigScope) -> Result<PathBuf, String> {
    match scope {
        PluginConfigScope::Project => Ok(crate::product::project_config_path()),
        PluginConfigScope::Global => Ok(crate::product::user_config_path()),
    }
}

fn write_config_acl(path: &Path, document: &a3s_acl::Document) -> Result<(), String> {
    ...
    let raw = a3s_acl::generate_acl(document);
    let temporary = path.with_extension(format!("acl.{}.tmp", std::process::id()));
    ...
    drop(file);
    if let Err(error) = activate_config_file(&temporary, path) {
        let _ = fs::remove_file(&temporary);
        return Err(format!(
            "Failed to activate config {}: {error}",
            path.display()
        ));
    }
    Ok(())
...
#[cfg(not(windows))]
fn activate_config_file(temporary: &Path, destination: &Path) -> std::io::Result<()> {
...
fn upsert_plugin_config(path: &Path, plugin: &PluginConfig) -> Result<(), String> {
    let mut document = if path.exists() {
        ...
        a3s_acl::parse_acl(&raw)
            .map_err(|error| format!("Config {} is invalid A3S ACL: {error}", path.display()))?
    } else {
        a3s_acl::Document {
    ...
    let [browser] = document.blocks.as_mut_slice() else {
        return Err(format!(
            "Config {} must contain exactly one browser block",
            path.display()
        ));
    };
    if browser.name != "browser" || !browser.labels.is_empty() {
        return Err(format!(
            "Config {} must contain an unlabeled browser block",
            path.display()
        ));
    }
    browser
        .blocks
        .retain(|block| block.name != "plugin" || block.labels.first() != Some(&plugin.name));

    let mut attributes = HashMap::new();
    attributes.insert(
        "command".to_string(),
        a3s_acl::Value::String(plugin.command.clone()),
    );
    attributes.insert(
        "args".to_string(),
        a3s_acl::Value::List(
    ...
    if let Some(source) = &plugin.source {
        attributes.insert("source".to_string(), a3s_acl::Value::String(source.clone()));
    }
    browser.blocks.push(a3s_acl::Block {
        name: "plugin".to_string(),
        labels: vec![plugin.name.clone()],
        blocks: Vec::new(),
        attributes,
    });
    write_config_acl(path, &document)
}
    ...
    match sub {
        ...
        "run" => {
            ...
            if is_core_plugin_entrypoint(request_type) {
                print_plugin_error(
                    &format!(
                        "plugin run cannot invoke core plugin entrypoint '{}'; use the dedicated a3s use browser command path",
                        request_type
                    ),
                    json_output,
                );
...
mod tests {
    ...
    fn plugin_config_upsert_replaces_existing_plugin() {
        ...
        let path = dir.path().join("browser.acl");
        fs::write(
            &path,
            r#"browser {
  headed = true
...
"#,
        ...
        let document = a3s_acl::parse_acl(&fs::read_to_string(&path).unwrap()).unwrap();
        let browser = &document.blocks[0];
        assert_eq!(browser.attributes["headed"], a3s_acl::Value::Bool(true));
        let plugins = browser
            .blocks
            .iter()
            .filter(|block| block.name == "plugin")
            .collect::<Vec<_>>();
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].labels, ["captcha"]);
        assert_eq!(
            plugins[0].attributes["command"],
            a3s_acl::Value::String("npx".to_string())
        );
        assert_eq!(
            plugins[0].attributes["source"],
            a3s_acl::Value::String("npm:agent-browser-plugin-captcha".to_string())
        );
    }
