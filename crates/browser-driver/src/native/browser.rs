        ...
        if input.chars().all(|c| c.is_ascii_digit()) {
            return Err(format!(
                "Expected a tab id like `t{}` or a label; positional integers are not accepted \
                 (run `a3s use browser tab` to list stable tab ids)",
                input
            ));
        }
        ...
        match tab_ref {
            TabRef::Id(id) => {
                if self.has_tab_id(*id) {
                    ...
                    Err(format!(
                        "Tab {} not found; run `a3s use browser tab` to list open tabs",
                        format_tab_id(*id)
                    ))
                }
            }
            TabRef::Label(name) => self
                ...
                .ok_or_else(|| {
                    format!(
                        "No tab with label `{}`; run `a3s use browser tab` to list open tabs",
                        name
                    )
                }),
        }
