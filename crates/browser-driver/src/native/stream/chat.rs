...
fn load_skills() -> Vec<(String, String)> {
    SKILL_NAMES
        .iter()
        .filter_map(|name| {
            crate::skills::find_skills_dirs()
                .into_iter()
                .map(|directory| directory.join(name).join("SKILL.md"))
...
pub(crate) fn get_system_prompt() -> &'static str {
    ...
    PROMPT.get_or_init(|| {
        ...
        format!(
            r#"You are an AI assistant that controls a browser through A3S Use Browser. You have an active browser session, but you can also create new sessions.
...
- Do not add `--json`.
- Do not run programs outside `a3s use browser`.
...
- To create a new session: add `--session <name>` to any command (e.g. `a3s use browser --session my-session open https://example.com`). If the session does not exist, it will be created automatically.
- To use a different browser engine: add `--engine <engine>` (e.g. `a3s use browser --session lp-session --engine lightpanda open https://example.com`). Supported engines: chrome (default), lightpanda.

The following skill references describe A3S Use Browser capabilities in detail. Use them when deciding which commands to run and how to approach tasks.
{sections}"#,
        )
    })
}

pub(crate) const CHAT_TOOLS: &str = r#"[{"type":"function","function":{"name":"agent_browser","description":"Execute an A3S Use Browser command. Runs against the active session by default. Add --session <name> to target or create a different session, and --engine <engine> to choose a browser engine.","parameters":{"type":"object","properties":{"command":{"type":"string","description":"The command to execute, e.g. 'a3s use browser open https://google.com' or 'a3s use browser --session new-session open https://example.com' or 'a3s use browser snapshot -i' or 'a3s use browser click @e3'"}},"required":["command"]}}}]"#;
...
pub(crate) async fn execute_chat_tool(session: &str, command: &str) -> String {
    ...
    let stripped = single
        .strip_prefix("a3s use browser ")
        .or_else(|| single.strip_prefix("agent-browser "))
        .unwrap_or(single);
    ...
    if !ALLOWED_COMMANDS.contains(&first_cmd) {
        return format!(
            "Blocked: '{}' is not a valid a3s use browser command.",
            first_cmd
        );
    }
...
mod tests {
    ...
    #[test]
    fn chat_loads_all_packaged_runtime_skills_from_the_a3s_directory() {
        ...
        guard.set(
            "A3S_USE_BROWSER_SKILLS_DIR",
            concat!(env!("CARGO_MANIFEST_DIR"), "/skill-data"),
        );
        guard.remove("AGENT_BROWSER_SKILLS_DIR");
        ...
        assert!(skills
            .iter()
            .all(|(_, content)| content.contains("a3s use browser")));
    }
}
