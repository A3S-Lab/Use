...
fn get_auth_dir() -> PathBuf {
    crate::product::state_root().join("auth")
}
...
const ENCRYPTION_KEY_ENV: &str = "A3S_USE_BROWSER_ENCRYPTION_KEY";
const LEGACY_ENCRYPTION_KEY_ENV: &str = "AGENT_BROWSER_ENCRYPTION_KEY";
const KEY_FILE_NAME: &str = ".encryption-key";

fn get_browser_state_dir() -> PathBuf {
    crate::product::state_root()
}

fn get_key_file_path() -> PathBuf {
    get_browser_state_dir().join(KEY_FILE_NAME)
}
...
/// Read the encryption key from the A3S environment or state root.
fn get_encryption_key() -> Result<Vec<u8>, String> {
    if let Ok(key_hex) =
        std::env::var(ENCRYPTION_KEY_ENV).or_else(|_| std::env::var(LEGACY_ENCRYPTION_KEY_ENV))
    {
...
fn ensure_encryption_key() -> Result<Vec<u8>, String> {
    ...
    let _ = writeln!(
        std::io::stderr(),
        "[a3s use browser] Auto-generated encryption key at {} -- back up this file or set {}",
        key_file.display(),
        ENCRYPTION_KEY_ENV
    );
