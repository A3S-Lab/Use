...
/// The release binary embeds the real Dashboard. Silently creating a
/// placeholder would make a broken release look healthy, so missing assets are
/// a hard build error.
fn require_dashboard_assets() {
    let dashboard_out = Path::new("dashboard/out");
    println!("cargo:rerun-if-changed=dashboard/out");
    if !dashboard_out.join("index.html").exists() {
        panic!(
            "A3S Use Browser Dashboard assets are missing; run 'pnpm --dir dashboard install && pnpm --dir dashboard build'"
        );
    }
}
