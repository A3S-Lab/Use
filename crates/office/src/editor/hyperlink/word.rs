...
pub(super) fn set(
    ...
    if !paragraph_path.starts_with("/body/") {
        return Err(editor_error(
            "use.office.mutation_path_unsupported",
            "Native Word hyperlink mutation currently supports document-body paragraphs.",
        ));
    }
    ...
    let paragraph = locate_word_path(&index, &paragraph_path)?;
    ...
    let existing = existing_path
        .as_deref()
        .map(|existing| locate_word_path(&index, existing))
        .transpose()?;
    ...
    let relationship_id = match &hyperlink.target {
        NativeOfficeHyperlinkTarget::External { uri } => {
            existing_external_relationship(package, DOCUMENT_PART, old_id.as_deref(), uri)?
                .map(Ok)
                .unwrap_or_else(|| add_external_relationship(package, DOCUMENT_PART, uri))?
                .into()
        }
    ...
    let link_path = if let Some(existing_path) = existing_path {
        ...
        package.set_part(DOCUMENT_PART, insert_child(&part, paragraph, fragment)?)?;
        format!("{paragraph_path}/hyperlink[{position}]")
    };
    remove_relationship_if_unused(package, DOCUMENT_PART, old_id.as_deref())?;
    Ok(link_path)
}
...
pub(super) fn remove(package: &mut NativeOfficePackage, path: &str) -> UseResult<()> {
    if !path.starts_with("/body/") {
        return Err(editor_error(
            "use.office.mutation_path_unsupported",
            "Native Word hyperlink removal currently supports document-body paths.",
        ));
    }
    let part = package.xml_part(DOCUMENT_PART)?;
    let index = index_xml(&part)?;
    let hyperlink = locate_word_path(&index, path)?;
...
fn update_existing(
    ...
    let index = index_xml(part)?;
    let existing = locate_word_path(&index, path)?;
    ...
    let edited = LosslessXmlPart::parse(DOCUMENT_PART.to_string(), bytes)?;
    let edited_index = index_xml(&edited)?;
    let edited_link = locate_word_path(&edited_index, path)?;
