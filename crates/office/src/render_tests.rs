...
#[tokio::test]
async fn word_html_is_deterministic_escaped_and_path_free() {
...
#[tokio::test]
async fn spreadsheet_html_keeps_observed_cells_sparse() {
    let temp = tempfile::tempdir().unwrap();
...
#[tokio::test]
async fn svg_rejects_word_and_spreadsheet_with_a_stable_error() {
    let temp = tempfile::tempdir().unwrap();
    for extension in ["docx", "xlsx"] {
        ...
        let document = NativeOfficeEditor::create(&path)
            ...
            .unwrap();
        assert_eq!(
            document.svg_view().unwrap_err().code,
...
async fn renderer_rejects_corrupt_internal_raster_parts() {
    ...
    let error = document.html_view().unwrap_err();
