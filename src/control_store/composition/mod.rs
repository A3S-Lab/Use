...
use super::effect_owner::capability_plane::{
    ...
    ProductionControlInvocationFactory, RecordingControlRuntimeServiceReadiness,
};
...
use crate::plugin_runtime::{
    ...
    RuntimeSurfacePlanStore, RuntimeSurfaceResolver,
};
...
pub(super) const CAPABILITY_RETENTION_CURSOR_ERROR: &str =
    "use.control.capability_payload_retention_cursor_stale";
pub(super) const CAPABILITY_RETENTION_SNAPSHOT_ERROR: &str =
    "use.control.capability_payload_retention_snapshot_missing";
pub(super) const CAPABILITY_GATEWAY_DRAIN_BINDING_ERROR: &str =
...
#[derive(Clone)]
pub(crate) struct ControlStoreRuntimeComposition {
    store: ControlStore,
    ...
    runtime_resolver: Arc<dyn RuntimeSurfaceResolver>,
    runtime_bindings: RuntimeBindingStore,
    /// Bind-time live loopback map shared with the production Gateway endpoint
    /// router. Opaque `gateway:` identities stay on receipts; URLs stay here.
    #[cfg(feature = "mcp")]
    endpoint_routes: Arc<ControlGatewayEndpointRouteTable>,
...
impl ControlStoreRuntimeComposition {
    ...
    pub(in crate::control_store) fn from_extension_paths(
        ...
        let runtime_resolver: Arc<dyn RuntimeSurfaceResolver> = Arc::new(
            CommittedRuntimeSurfaceResolver::new(runtime_source, dependencies.runtime_registry),
        );
        let runtime_bindings = RuntimeBindingStore::for_control_authority(paths);
        #[cfg(feature = "mcp")]
        let endpoint_routes = Arc::new(ControlGatewayEndpointRouteTable::new());
        #[cfg(feature = "mcp")]
        let runtime_readiness: Arc<dyn ControlRuntimeServiceReadinessPort> =
            Arc::new(RecordingControlRuntimeServiceReadiness::new(
                dependencies.runtime_readiness,
                Arc::clone(&endpoint_routes),
            ));
        #[cfg(not(feature = "mcp"))]
        let runtime_readiness = dependencies.runtime_readiness;
...
fn runtime_prepare_identity(
    ...
    // Package-local native launchers are owned by the static surface host.
    // Runtime plan publications exist only for release-backed Runtime providers.
    if provider_id == NATIVE_RUNTIME_PROVIDER_ID {
        return None;
    }
...
/// Built-in package-local Tool/MCP launcher; not a Runtime plan payload owner.
const NATIVE_RUNTIME_PROVIDER_ID: &str = "a3s-use-native-launcher";

#[cfg(feature = "mcp")]
pub(in crate::control_store) use gateway::{
    ControlCapabilityGatewayCutoverActivation, ControlCapabilityGatewayReconciliation,
};
