//! Owned filesystem protocol shared by Control restore staging and activation.
...
use super::control_restore_evidence::MAX_CONTROL_CANDIDATE_BYTES;
...
pub(super) const CANDIDATE_FILE: &str = CONTROL_STORE_DATABASE_FILE;
pub(super) const PARTIAL_FILE: &str = "control.sqlite3.partial";
pub(super) const EVIDENCE_FILE: &str = "candidate.json";
pub(super) const EVIDENCE_PARTIAL_FILE: &str = "candidate.json.partial";
pub(super) const MAX_EVIDENCE_BYTES: u64 = 128 * 1024;

pub(super) async fn publish_evidence(directory: &Path, expected: &[u8]) -> UseResult<()> {
...
pub(super) async fn validate_entries(directory: &Path) -> UseResult<()> {
...
pub(super) async fn remove_partial_family(directory: &Path) -> UseResult<()> {
    for name in [
        PARTIAL_FILE,
        "control.sqlite3.partial-wal",
        "control.sqlite3.partial-shm",
...
pub(super) async fn require_no_sidecars(directory: &Path) -> UseResult<()> {
...
pub(super) async fn cleanup_quiescent_sidecars(candidate: &Path) -> UseResult<()> {
...
pub(super) async fn any_sidecar(directory: &Path) -> UseResult<bool> {
    for name in [
        "control.sqlite3-wal",
        "control.sqlite3-shm",
        "control.sqlite3-journal",
...
pub(super) async fn hash_owned_file(path: &Path) -> UseResult<(u64, String)> {
...
pub(super) async fn read_exact_owned(
    path: &Path,
    expected: u64,
    maximum: u64,
) -> UseResult<Vec<u8>> {
