...
/// Typed lifecycle ports injected by a host factory.
///
/// Factories assemble providers and journals here. The Use engine alone turns
/// the set into a [`PluginLifecycleCoordinator`]; factories must not construct
/// coordinators directly.
#[derive(Clone)]
pub struct LifecycleProviderSet {
    journal: PluginLifecycleJournalStore,
    hosts: PluginLifecycleHosts,
}

impl LifecycleProviderSet {
    pub fn new(journal: PluginLifecycleJournalStore, hosts: PluginLifecycleHosts) -> Self {
        Self { journal, hosts }
    }

    /// Engine-owned coordinator construction from injected ports.
    pub fn into_coordinator(self) -> PluginLifecycleCoordinator {
        PluginLifecycleCoordinator::new(self.journal, self.hosts)
    }

    pub fn hosts(&self) -> &PluginLifecycleHosts {
        &self.hosts
    }
}
...
include!("coordinator_execute.rs");
...
include!("coordinator_helpers.rs");
