...
struct RecordingHost {
    ...
    durable_graph_root: Option<std::path::PathBuf>,
    crash_after_graph_effect: Mutex<Option<String>>,
}
...
impl PluginGraphCapabilityLifecycleHost for RecordingHost {
    async fn publish_capabilities_with_cutover(
        ...
        *drift = false;
        let publication = PluginGraphCapabilityPublication::new(
            packages,
            PluginCapabilityCutoverEvidence::new(
                generation_before,
                generation_before + 1,
        ...
        durable_graph_host::record_graph_effect(self, "publish", key, &publication).await?;
        Ok(publication)
    }
    ...
    async fn hide_capabilities_with_cutover(
        ...
        durable_graph_host::record_graph_effect(self, "hide", key, &publication).await?;
        Ok(publication)
    }
