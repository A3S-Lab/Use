...
fn authorized_binding(
    ...
    let default_binding = PluginOperationPlanBinding {
        // Include created_at_ms so uninstall→reinstall does not collide with a
        // prior completed Control operation that used the same lock digest.
        operation_id: format!("{operation}:package-graph:{identity}:{created_at_ms}"),
        created_at_ms,
        expires_at_ms,
        scope: scope.clone(),
        authority,
