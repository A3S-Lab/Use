...
Model acquisition uses an immutable A3S OCR release bundle instead of the
upstream model CDN. It bounds connection setup and stalled reads rather than
total transfer duration. Interrupted bodies resume only from an exact
validated byte range; a range-ignoring server restarts the staging file. The
exact bundle length and SHA-256 gate activation, and extraction admits only the
four declared model assets.
