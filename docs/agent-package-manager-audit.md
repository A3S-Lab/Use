...
| Generation-safe upgrade and drain | Immutable session factory, snapshot leases, list-change hub, explicit retention plans, paired catalog/descriptor retention coordinator, durable Control cursor reopening, and an internal lease guard that follows cloned Gateway servers | Mechanism qualified | Lifecycle must publish the new catalog, call the Control-bound session replacement seam, retain old leases, and retire payloads in one production transition |
