...
| P3 — Blue/green retirement | Compose the implemented package/Runtime dual-generation stores with Gateway/projection/grant receipts, then durably cut over, automatically roll back or hide/drain/remove N, and garbage-collect safe state | Failed N+1 leaves N callable; successful N+1 leaks no old Runtime unit or route |
