...
| P0-G — Dependency-graph upgrade core (complete 2026-08-05) | Exact prior/candidate locks, Add/Replace/Retain planning, dependency-forward N+1 preparation, one capability cutover, automatic unpublished-candidate rollback, reverse N retirement, exact graph CAS, and durable replay | Preparation/publication failure restores N and removes additions; successful cutover retires replacements exactly once; applying/rolling-back crash fixtures converge without generation inflation |
...
| P3 — Production blue/green composition (core complete; providers/GC pending) | Compose the implemented graph cutover, rollback, and retirement coordinator with Gateway/Knowledge/grant receipts; admit separately reviewed dependency-removal transitions and garbage-collect only unreferenced state | Failed N+1 leaves N callable across every production provider; successful N+1 leaks no old Runtime unit or route; removed dependency nodes survive crash-safe GC |
...
P1, P2, and the remaining provider-composition/GC work in P3 remain release
