...
typed production Service/Gateway provider composition, local CLI/Web/TUI
reviewed-enablement presentation, and a real A3S Knowledge backend. Umbrella
...
enable/disable, upgrade, uninstall, and reinstall. Schema-v3 packages now
persist that state and the complete operation request/result under
operation- and package-scoped cross-process locks, resume lifecycle checkpoints
after a crash, and replay a completed operation without touching the package
graph. Every changed toggle now binds operation-plan v4 to an exact retained
artifact, receipt, scope, state revision, capability generation, and derived
OKF/secret/Grant impact. With an injected authorization provider,
permission-bearing enable prepares the exact Workspace Grant before atomic
...
permission-free compatibility path. Local CLI/Web/TUI still need to expose the
same reviewed two-step UX.
...
- exact umbrella/managed-host Grant snapshot, two-pass policy, canonical
...
- expose the Use-generated immutable enablement plan plus exact confirmation in
  the local Code CLI/Web/TUI UX using one shared presentation service;
...
| P4 — Product adapters (Code graph and managed-host reviewed enablement complete; local UX/providers pending) | Preserve the shared CLI/Web Marketplace journal, Code snapshot watcher, local Flow runtime, Use-owned enablement, and fenced managed-host v4 path; add one shared local reviewed enablement presentation service, visible Web Flow controls, and production provider adapters | Detached Web API covers install/run/upgrade/run/uninstall/restart with retained Flow history and TUI covers local routing plus watcher readiness; managed permission-bearing review is restart-safe; local reviewed toggles, visible Web UX, and production hosts must extend the same no-restart proof |
...
P1, the local Code CLI/Web/TUI reviewed-enablement presentation beyond the
completed managed-host protocol, and the remaining provider-composition work in
P3 remain release blockers for calling schema-v3 cognitive-package lifecycle production-ready.
...
Grants. The standalone manager derives and durably binds canonical Grant inputs
after trusted authority and exact confirmation, then selects this path for
...
hosts and forwards the same enablement plan/confirmation evidence through one
shared local Code CLI/Web/TUI presentation service. Fenced managed hosts already
use the versioned capability-v4 contract.
