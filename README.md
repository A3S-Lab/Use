...
| UI | Integrity-bound static entry point | Code Web publishes and adopts exact generation/revision-bound sandbox documents, transfers a dedicated v2 `MessagePort`, ignores ambient messages, terminates self-navigation, identity-binds reviewed context, and drains/replaces old frames on Registry changes. Reviewed backend bindings, persistent package state, failed-N+1 fallback, and native-host rendering remain host work |
...
| A3S Code CLI/TUI/Web integration | Reviewed Runtime Task install, offline restart disable/re-enable, apply-time build drift rejection, watcher hot-plug, Web marketplace lifecycle, generation-bound Activity documents, v2 MessagePort browser composition and context review, and TUI `/packages` review are tested; release qualification remains |
