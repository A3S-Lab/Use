...
| Capability Gateway contract and embedding MCP adapter | Implemented and contract-tested: immutable path-free descriptor/catalog contracts, opaque invocation/artifact/endpoint references, exact snapshot-lease and publication/lifecycle-generation binding, and a standard MCP `CapabilityGatewayMcpServer` that routes only catalog-authorized tools through an injected `CapabilityGatewayInvocationProvider`. The host can expose Streamable HTTP at `/mcp` with bearer authentication, optional exact Origin policy, duplicate-header rejection, bounded in-flight/rolling-window admission, sanitized HTTP errors, an explicit required pre-invocation authorization hook, and a typed host-authenticated transport/principal context delivered to both authorization and invocation. `CapabilityGatewayHttpConfig::for_principal` binds one configured principal to a validated endpoint credential. `CapabilityGatewayInvocationResolver` and `CapabilityGatewayResolvedProvider` now provide a single-resolution, lease-scoped host path for opaque references; the returned handle must retain the exact package-generation lease through invocation. An independent Rust client discovery/invocation check covers the path-free boundary. Multi-principal token/consumer wiring, CLI wiring, TLS termination, and the TypeScript/Python client/recovery matrix remain open |
