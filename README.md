...
| UI | Integrity-bound static entry point | Code Web publishes and adopts exact generation/revision-bound sandbox documents, transfers a dedicated v3 `MessagePort`, ignores ambient messages, terminates self-navigation, identity-binds reviewed context, and drains/replaces old frames on Registry changes. Code owns bounded durable state per scope/package/surface, authorizes every request against the exact published lifecycle generation, retains state across restart, disable, rollback, and upgrades that retain the surface, and clears state on true surface removal. Before cutover, Code Web loads exact path-free N+1 bytes in a hidden authority-free sandbox and accepts only `activity.ready`; load, navigation, protocol, or timeout failure keeps N selected, rolls N+1 back without residue, and makes the failed plan non-replayable. Reviewed backend bindings, equivalent readiness outside Code Web, and native-host rendering remain host work |
...
| A3S Code CLI/TUI/Web integration | Reviewed Runtime Task install, offline restart disable/re-enable, apply-time build drift rejection, watcher hot-plug, Web marketplace lifecycle, generation-bound Activity documents, v3 MessagePort browser composition, Code Web failed-N+1 candidate readiness/rollback/fresh-plan retry, durable UI state, context review, and TUI `/packages` review are tested; CLI/TUI browser readiness and release qualification remain |
