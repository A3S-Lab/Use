...
Tool, MCP, OKF, Skill, and UI have a schema-v3 contract baseline. M0K-B implements the named OKF manifest, recursive package validation, catalog-v3 evidence, plan-v2 impact, Knowledge receipt/observation/projection contracts, and dependency-gated reconciliation. The persistent A3S Knowledge adapter and scope-aware host projection remain pending, so missing promoted evidence stays unpublished.
