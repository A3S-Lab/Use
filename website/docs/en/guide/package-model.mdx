...
Tool, MCP, OKF, Flow, Skill, and UI have a schema-v3 contract baseline. P0 adds generation-bound package commit/removal, atomic capability publish/hide, route-lease drain, and legacy-bypass rejection. The package dependency foundation adds bounded SemVer resolution, an exact Registry/TUF-bound lock, dependency-forward download and preparation, retained shared dependencies, one graph cutover, reverse uninstall, and durable crash replay. Flow adds a fixed `a3s-flow` engine, Native TypeScript source evidence, Tool/MCP/OKF dependency closure, lifecycle ordering, and typed catalog projection. A3S Code now composes executable Tool Tasks, stdio MCP, the real Flow preflight host, Skill, and UI; TUI/Web consume one exact-generation watcher and Web verifies install, upgrade, and uninstall replacement without restart. M0K-B/M0K-C-A implement OKF control-plane and lifecycle foundations. Production Knowledge, Runtime Service, HTTP MCP/Gateway, durable Flow execution, and prior-generation retirement remain pending, so missing host evidence stays unpublished.
