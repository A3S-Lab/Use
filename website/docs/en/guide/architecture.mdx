...
reference authorization. `CapabilityGatewayHttpConfig::for_principal` now binds
one host-configured principal to the validated bearer credential and passes a
typed transport/principal context to both provider hooks; the value is never
serialized into MCP discovery or results.
...
policy explicitly. The hook does not replace live reference resolution or a
multi-principal token registry.
