...
The inactive schema-v10 kernel now derives that complete outbox inventory from
...
It writes one canonical content-addressed Index document from the committed
candidate and exact terminal surface evidence, without a second SQLite store or
mutable current file; Control's applied cutover observation is the sole
publication cursor. Admission reads that cursor before and after taking shared
locks for every exact package lifecycle incarnation. Drain requires the old
incarnation to be unpublished and takes its exclusive lock, safely deferring
while an accepted call is active. No-follow, no-replace publication and exact
staging replay protect the immutable Index. Index and lease files are derived
operational state excluded from backup; restore must rebuild the Index from
verified Control evidence. A real composition test covers Knowledge, Skill,
...
payload bytes, not desired state, so lifecycle conversion must publish exact
new records before the corresponding Control commit. The process-local Runtime
selection used for qualification is not production authority.
The qualification composition now has one reviewed-operation-only entry point.
Control projects graph, Grant, provider, capability, and effect fields from the
durable operation, checks exact Runtime prepare coverage and reviewed Grant
proposal digests, then publishes immutable plan bytes before committing the
projected generation under one shared installation fence. This is an internal
cutover proof, not a public API or production lifecycle wiring.
...
unknown outcomes remain reconciliation evidence. None is desired state. An
