...
fence and active marker, preserves that marker and current operation, replaces
only terminal history, and binds exact before/source/target inventories in
durable activation evidence. Replay tolerates current-operation status progress
but rejects marker, candidate, retired-record, or publication-partial drift.
Marker-only handoff and native 64-record retention remain deterministic.
...
rebinding, and candidate drift fail closed. Control activation now durably
records top-level intent before live publication. The immutable attempt remains
the restore identity; `activation.json` is the sole mutable ordered journal,
and the global `.maintenance.restore.json` marker binds the same operation and
blocks ordinary shared access. The fixed order is journal, marker, owner effect,
then checkpoint. Preflight rejects invalid state before intent, while semantic,
physical, sidecar, and attempt checks run again after the marker before atomic
no-clobber publication. Reopen reacquires the exact fence, rebinds the same
verified snapshot, attempt, and policy, reconstructs present unactivated owner
candidates, and converges journal/marker temporary and
post-publication/pre-checkpoint boundaries. Missing or ambiguous markers after
an effect, links, rebinding, and evidence drift fail closed. The Control
checkpoint binds the canonical path-free result, completed replay does not
rewrite it, and the global marker remains active. Cross-owner checkpoints and
activation order, marker retirement, full subprocess-exit qualification,
