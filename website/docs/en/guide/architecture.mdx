...
preserving existing fields. Explicit reviewed CLI apply, read-side and
enable/disable convergence, TUI migration, and Code-side manager MCP
...
- `src/plugin_manager`: the shared typed application service and standard
  ...
  Registry-backed install/upgrade/uninstall mutations use this service;
  explicit reviewed CLI apply, the remaining CLI read/enablement paths, TUI,
  and Code MCP composition are still pending.
