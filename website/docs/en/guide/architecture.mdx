...
Runtime Tool/MCP, immutable Flow/Skill/UI evidence, and OKF Knowledge adapters are implemented. The P0 schema-v3 package/capability hosts commit a verified generation as installed-disabled, publish or hide exact-generation snapshots, drain route leases, and remove only receipt-owned roots. The dependency foundation adds exact retained-node verification, crash-safe atomic graph publication, durable admission/lock records, and journal recovery after partial install or uninstall. Signed remote standalone CLI installs use this graph. Umbrella Plugin Manager and grant composition, the production A3S Flow/Knowledge hosts, Gateway integration, and prior Runtime generation retirement remain in progress, so this foundation does not claim a complete production saga.
