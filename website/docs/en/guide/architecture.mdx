...
Runtime Tool/MCP, immutable Skill/UI, and OKF Knowledge adapters are implemented. The P0 schema-v3 package/capability hosts commit a verified generation as installed-disabled, publish or hide exact-generation snapshots, drain route leases, and remove only receipt-owned roots. The dependency foundation adds exact retained-node verification and crash-safe atomic graph publication. Umbrella Plugin Manager and grant composition, the real A3S Knowledge backend, Gateway integration, and prior Runtime generation retirement remain in progress. The legacy CLI does not yet expose graph installation, so this foundation does not claim a complete production saga.
