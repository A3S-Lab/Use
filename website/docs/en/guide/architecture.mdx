...
path-free receipt is bound to the exact Control export, but observation restore
is not implemented yet. These remain inactive owner primitives; the Host
projection and Restore Coordinator adapters, observation restore, complete-set
coordination, full process-exit qualification, production orchestration, and
