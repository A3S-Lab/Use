...
rebinding, and candidate drift fail closed. Control also owns a separate
clean-target publication primitive under the still-retained exact attempt and
guard. It rechecks semantic export, physical evidence, and sidecar absence,
publishes without replacement, and reconciles exact post-publication replay to
one path-free result. Ambiguous or drifted candidate/live state fails closed.
Complete-set activation, its durable cross-owner journal and full
subprocess-exit qualification, production backup/restore wiring, and
coordinated authority cutover remain disabled until every consumer can switch
together.
