...
> Current status: OKF is accepted product scope. Its manifest contract, canonical fixtures, receipt/projection, and A3S Knowledge atomic-index protocol are not frozen yet. The current schema-v3 parser does not accept an undefined `okf` block.
...
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Skill       | A Skill teaches an agent how to perform work; OKF supplies citable domain facts, decisions, and concepts. The target graph may let a Skill require one named OKF generation. |
...
- Paths, links, file count, expanded bytes, and parsed-node size are bounded. A safe dangling link is diagnostic; a reference that resolves outside the package boundary is rejected.
