...
> Current status: the standalone lifecycle includes a bundled cross-platform SQLite/FTS5 Knowledge backend, complete User/Workspace isolation, transactional stage/promote/remove, exact projection-authorized cited search, restart recovery, bounded storage quota/retention/GC, and signed install/query/upgrade/uninstall coverage. A3S Code/Web-managed Workspace and session carriers, leased-query qualification, and operational backup/repair remain pending; without exact promoted evidence, the surface is not published.
...
The report includes retained projections, tombstones, expanded bytes, active policy limits, allocated database bytes, and reclaimable bytes. Workspace diagnostics require an explicit `--scope-id`; the CLI never guesses a current Workspace identity. These controls harden the standalone backend; they do not claim that Code/Web managed carriers or whole-product backup and repair are complete.
