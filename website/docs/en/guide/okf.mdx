...
> Current status: the standalone lifecycle includes a bundled cross-platform SQLite/FTS5 Knowledge backend, complete User/Workspace isolation, transactional stage/promote/remove, exact projection-authorized cited search, restart recovery, bounded storage quota/retention/GC, scope-local integrity audit, verified database backup, authority-preserving FTS repair, and signed real-process coverage. A3S Code/Web-managed leased-query qualification, coordinated restore, and whole-product recovery remain pending; without exact promoted evidence, the surface is not published.
...
Repair never edits projection receipts, selected generations, bindings, lifecycle evidence, or Grants. Copying a backup into live state is unsupported: coordinated restore and whole-product recovery remain release gates. See the [OKF Knowledge operations runbook](https://github.com/A3S-Lab/Use/blob/main/docs/okf-knowledge-operations.md).
