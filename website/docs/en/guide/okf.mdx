...
Inspect the default User scope or an exact Workspace scope without reading concept content:
...
The report includes retained projections, tombstones, expanded bytes, active policy limits, allocated database bytes, and reclaimable bytes. Workspace diagnostics require an explicit `--scope-id`; the CLI never guesses a current Workspace identity.
