...
In A3S Use, OKF is a first-class cognitive surface alongside Tool, MCP, Skill, and UI, but it is not an executable workload.

> Current status: M0K-B implements the frozen bundle descriptor, schema-v3 manifest, recursive package validation, catalog-v3 evidence, plan-v2 impact, projection receipt, Knowledge observation, capability projection, dependency closure, and reconciler gate. The persistent A3S Knowledge stage/promote/remove adapter and scoped cited-retrieval path remain pending; without promoted evidence, the surface is not published.
