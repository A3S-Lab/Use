...
The installers require Cosign and authenticate `checksums.txt` against the exact A3S Use tag workflow identity and GitHub OIDC issuer before downloading a platform archive. Invalid or missing evidence fails closed, and the verified manifest and bundle are retained with the installed version. The installer script remains a separate bootstrap trust boundary and should be reviewed or distributed through a trusted system package. GitHub attestation verification remains an additional operator check. Deterministic archive serialization is not proof that native binaries reproduce across independent clean builders; that comparison and off-Release evidence retention remain release gates.
