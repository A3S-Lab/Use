...
The default installer still obtains `checksums.txt` and the archive from the same GitHub Release HTTPS boundary. An operator who needs independent evidence must verify the Sigstore bundle, selected checksum, and GitHub attestation before executing the installer. Deterministic archive serialization is not proof that native binaries reproduce across independent clean builders; that comparison and installer-enforced signature policy remain release gates.
