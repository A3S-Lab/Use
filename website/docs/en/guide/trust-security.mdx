...
Remote registries are named, host-owned ACL configuration. The default `a3s` identity is not a compiled endpoint or mandatory service: operators can bind it to a mirror or private registry, disable it without deleting trust state, or remove the override. Release bundles remain available independently of remote registries.
...
Only enabled sources participate in discovery and installation. Duplicate package identities across enabled registries fail as ambiguous. Replacement also never rewrites an installed receipt: upgrades remain bound to the recorded source name, URL, TUF root, channel, and target digest and fail closed after identity drift.
