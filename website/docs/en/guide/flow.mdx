...
A3S Use implements manifest admission, exact source evidence, signed catalog closure, package lifecycle ordering, reconciler ownership, host-capabilities v4, manager tools v3, typed capability projection, and a concrete `A3sFlowLifecycleHost`. That host delegates to the real `a3s-flow` Native TypeScript preflight, persists an exact-generation source/compiled-artifact binding, and reinspects both artifacts during observation. The standalone engine rejects required Flow packages before mutation when no `a3s-flow` host adapter is injected.
