...
A3S Use implements manifest admission, exact source evidence, signed catalog closure, package lifecycle ordering, reconciler ownership, host-capabilities v2, manager-toolset v3, and typed capability projection. The standalone engine rejects required Flow packages before mutation when no `a3s-flow` host adapter is injected.
...
Production compiler preflight, durable execution, run observation, Code TUI/Web integration, and the `flow.json` import/deployment adapter remain release gates. The [roadmap](./roadmap) tracks those gates explicitly.
