...
receipt v4, plan v4, host protocol v6, managed scope v2, manager tools v4,
pending graph v4, pre-lock resolution attempt/diagnostic v1, pre-plan download
attempt/diagnostic v1, and enablement state/operation v2. Superseded preview
...
published. The tagged `v0.3.2` attempt failed that gate on four targets, but
non-publishing qualification run
[32600882148](https://github.com/A3S-Lab/Use/actions/runs/32600882148)
subsequently passed all five targets. See
...
```bash
a3s-use plugin search research --kind tool --json
a3s-use plugin inspect acme/research --version 2.0.0 --json
...
a3s-use plugin apply-plan \
  ...
  --yes \
  --json
a3s-use plugin plan-disable acme/research --json
```
...
```bash
a3s-use extension diagnose acme/research --json
a3s-use extension diagnose acme/research --history --json
```

Workspace diagnostics additionally require
...
a3s-use knowledge backup-retention ./backups --json
a3s-use knowledge plan-restore ./user.a3s-okf-backup --json
a3s-use knowledge restore ./user.a3s-okf-backup \
  --plan-digest sha256:<reviewed-plan-digest> --yes --json
a3s-use knowledge restore-status --json
a3s-use state backup ./use-state.a3s-use-state-backup --json
a3s-use state verify-backup ./use-state.a3s-use-state-backup --json
a3s-use state backup-retention ./backups --max-backups 32 --max-bytes 2199023255552 --json
a3s-use state plan-restore ./use-state.a3s-use-state-backup --json
...
The separate `state backup` command captures installed package roots and every
allowlisted Registry, generation, Grant, binding, lifecycle/package-operation,
Knowledge, enablement, Host Manager, and Flow Runtime state family under one
exclusive maintenance fence. Its deterministic `a3s.use.state-backup.v1`
