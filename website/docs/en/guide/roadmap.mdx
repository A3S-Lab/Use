...
- Complete live server-side invocation-reference resolution, multi-principal
  ...
  recovery. The current embedding propagates an explicitly configured
  principal from HTTP bearer authentication into provider authorization and
  invocation; it is not yet a multi-principal registry or live resolver.
