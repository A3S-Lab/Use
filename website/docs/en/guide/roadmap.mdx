...
external owner and is not registered for backup/restore. Runtime Tool release
planning now carries canonical input/output-schema attestation through the
inactive Runtime and Control path; production Control/Runtime wiring, key
custody, and backup/restore registration remain open. The core now defines a
...
rotation, expiry, and revocation. This is a qualified mechanism only: the
Registry/TUF key source, Control proof admission, and production lifecycle
wiring remain open. The inactive Flow owner now follows the
