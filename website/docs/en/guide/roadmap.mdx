...
- The path-free `CapabilityDescriptor` and catalog contracts, opaque reference
  types, and embedding `CapabilityGatewayMcpServer` are implemented and
  contract-tested. The adapter publishes only catalog-authorized Tools over
  standard MCP through an injected provider; live-generation lease resolution,
  auth/rate limits, CLI wiring, and production-host recovery remain open.
...
- PR [#192](https://github.com/A3S-Lab/Use/pull/192) freezes the portable
  descriptor/catalog schemas and adds an embedding standard-MCP adapter.
- Complete server-side invocation-reference resolution, exact-generation
  leases across calls and streams, drain/retirement, bounded authentication and
  rate limits, and CLI/service wiring.
