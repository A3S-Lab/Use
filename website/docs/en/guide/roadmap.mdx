...
post-effect ambiguity remains unknown. Production lifecycle still does not
construct the dispatcher: it must reconstruct the complete Runtime plan after
restart from committed semantics-digest authority rather than use the
process-local selection retained by this qualification.
...
composition, backup/restore wiring, and coordinated authority cutover remain
open; no A2 roadmap item is complete yet.
...
- Add production Runtime Service and HTTP MCP/Gateway providers with a
  restart-safe resolver for the full committed Runtime plan.
