...
- The path-free `CapabilityDescriptor` and catalog contracts, opaque reference
  ...
  denial results. Live reference resolution, multi-principal consumer wiring,
  CLI wiring, TLS termination, and production-host recovery remain open.
...
- PR [#192](https://github.com/A3S-Lab/Use/pull/192) freezes the portable
  ...
  rejection and a real independent Rust client check.
- Complete live server-side invocation-reference resolution, multi-principal
  authorization wiring, CLI/service wiring, TLS deployment, and production
  recovery. The embedding now exposes `CapabilityGatewayInvocationResolver`
  and `CapabilityGatewayResolvedProvider`: one opaque reference is resolved to
  a private lease, checked against the catalog identity, authorized once, and
  held through invocation. Production receipt/Runtime/Grant composition and a
  multi-principal registry remain open.
