...
当前只接受一条预览协议基线。manifest v3、catalog v3、receipt v4、plan v4、
...
attempt/diagnostic v1，以及 enablement state/operation v2。过时的预览状态会被拒绝，
并提示清理后重新安装。
...
尝试在四个 target 上未通过该门禁，但后续非发布资格运行
[32600882148](https://github.com/A3S-Lab/Use/actions/runs/32600882148)
已让全部五个平台通过。剩余 bootstrap 边界与可选 GitHub attestation 验证见
...
独立的 `state backup` 命令会在同一个独占 maintenance fence 内捕获已安装
package root，以及所有 allowlisted Registry、generation、Grant、binding、
lifecycle/package operation、Knowledge、enablement、Host Manager 与 Flow Runtime
state family。确定性的 `a3s.use.state-backup.v1` manifest 只记录可移植相对路径、
