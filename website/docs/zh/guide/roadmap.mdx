...
- 无路径的 `CapabilityDescriptor` 与 catalog 合约、opaque reference 类型以及
  ...
  并清理拒绝结果；多 principal consumer wiring、CLI wiring、TLS 部署与
  production-host recovery 仍未完成。
...
- PR [#192](https://github.com/A3S-Lab/Use/pull/192) 已冻结 portable
  ...
  独立 Rust client 检查。
- 仍需完成 live server-side invocation-reference 解析、多 principal
  authorization wiring、CLI/service wiring、TLS 部署与 production recovery。
  ...
  生产 receipt/Runtime/Grant 组合与多 principal registry 仍未完成。
  当前 embedding 已把 HTTP bearer authentication 显式配置的 principal 传给
  provider authorization 与 invocation；它还不是 multi-principal registry
  或 live resolver。
