...
- 无路径的 `CapabilityDescriptor` 与 catalog 合约、opaque reference 类型以及
  ...
  只通过注入 provider，以标准 MCP 发布 catalog 授权的 Tool；实时 generation
  lease 解析、auth/rate limit、CLI wiring 与 production-host recovery 仍未完成。
...
- PR [#192](https://github.com/A3S-Lab/Use/pull/192) 已冻结 portable
  descriptor/catalog schema，并增加 embedding 标准 MCP adapter。
- 仍需完成 server-side invocation-reference 解析、跨 call/stream 的精确
  generation lease、drain/retirement、有界 authentication/rate limit 与
  CLI/service wiring。
