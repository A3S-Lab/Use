...
prepared source、不创建 lifecycle receipt，并让 reference admission 跨越独立 Control commit。仍待实现的是
Runtime、Flow、Capability Index、invocation lease adapter 与生产 composition。私有的无路径 payload 契约现已冻结五个外部 owner ID
...
fail closed。生产 Grant 转换、Runtime、Flow、Capability Index、invocation lease adapter 与 dispatcher 组合、backup/restore 接线和
