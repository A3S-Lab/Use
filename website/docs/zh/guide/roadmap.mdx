...
semantics digest 重建完整计划并重新校验精确 provider evidence。生产 lifecycle 仍未构造 dispatcher；
它必须提供持久化 host source 与原子的 composition，不能把本次验收使用的进程内 selection 当作生产权威。私有的无路径 payload
契约现已冻结五个外部 owner ID
...
fail closed。生产 Grant 转换、可重启的 Runtime plan 重建、Runtime/Flow dispatcher 组合、
backup/restore 接线和协调一致的权威切换仍未完成，因此尚未完成任何 A2 roadmap 项。
