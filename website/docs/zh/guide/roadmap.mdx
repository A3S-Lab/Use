...
后的歧义都保持 unknown。生产 lifecycle 仍未构造 dispatcher；它必须在重启后从已提交的 semantics digest
权威重建完整 Runtime plan，不能把本次验收使用的进程内 selection 当作生产权威。私有的无路径 payload
契约现已冻结五个外部 owner ID
...
- 增加生产 Runtime Service 与 HTTP MCP/Gateway provider，并提供可在重启后
  重建完整已提交 Runtime plan 的 resolver。
