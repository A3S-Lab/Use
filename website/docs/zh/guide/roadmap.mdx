...
- 无路径的 `CapabilityDescriptor` 与 catalog 合约、opaque reference 类型以及
  ...
  per-consumer authorization、CLI wiring、TLS 部署与 production-host recovery
  仍未完成。
...
- PR [#192](https://github.com/A3S-Lab/Use/pull/192) 已冻结 portable
  ...
  独立 Rust client 检查。
- 仍需完成 live server-side invocation-reference 解析、per-consumer
  authorization、CLI/service wiring、TLS 部署与 production recovery。
