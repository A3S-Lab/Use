...
Tool、MCP、OKF、Flow、Skill 与 UI 已有 schema-v3 contract baseline。P0 已增加 generation-bound package commit/removal、原子 capability publish/hide、route-lease drain 与 legacy-bypass rejection。包依赖基础已增加有界 SemVer 解析、精确 Registry/TUF 绑定 lock、依赖前向下载与准备、共享依赖保留、一次图切换、反向卸载和 durable crash replay。Flow 增加固定的 `a3s-flow` 引擎、Native TypeScript 源码证据、Tool/MCP/OKF 依赖闭包、生命周期顺序与类型化 catalog projection。A3S Code 现已组合 executable Tool Task、stdio MCP、真实 Flow preflight host、Skill 与 UI。CLI、TUI 与 Web 解析同一个精确 `flow.json` 身份并共用 workspace-local 持久 run；Web 已验证安装、运行、精确 generation 升级、卸载、历史保留与进程重启恢复。standalone OKF 现已使用 scope-isolated SQLite/FTS5 后端，具备事务化 cutover、精确 projection 授权带引用搜索，以及 signed install/query/upgrade/uninstall 覆盖。Code/Web 托管 Knowledge carrier、Runtime Service、HTTP MCP/Gateway、分布式 Flow 调度/恢复、retention 与旧 generation 退休仍待完成，因此缺少宿主证据时不会发布。
