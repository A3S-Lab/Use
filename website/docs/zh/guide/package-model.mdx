...
| 表面         | 含义                                           | 激活目标                                            |
| ------------ | ---------------------------------------------- | --------------------------------------------------- |
| Tool Task    | 一次性、非交互 CLI 工作负载                    | A3S Runtime Task 或宿主显式持有的 native adapter    |
...
当前只接受 schema v3。它定义可重复命名表面、包依赖、必需且有大小限制的 UTF-8 `README.md`，以及无环 readiness graph。过时的预览 manifest 和 receipt 会被拒绝，并提示清理后重新安装。所有路径都相对包根目录；激活前会拒绝路径穿越、链接、归档歧义、route 冲突、超限包、provenance 漂移和宿主版本不兼容。
