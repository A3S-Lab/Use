...
Tool、MCP、Skill 与 UI 已有 schema-v3 contract baseline。OKF 是最新目标架构中的一等认知表面，正式 manifest contract、fixture 和 host projection 尚待冻结；当前 parser 不接受尚未定义的 `okf` block。

OKF 不是可执行 workload。它是一个可共享知识包：每个非保留概念是带 YAML frontmatter 的 UTF-8 Markdown，文件路径是概念身份，标准 Markdown links 形成知识图，`type` 是必需字段。第一次冻结以 OKF v0.2 为目标，并明确兼容 v0.1。原始 PDF、Office、图片或网页不直接作为 OKF 权威内容；独立 compiler 必须先生成合规 bundle。
...
仓库包含可执行 fixture：[`plugin-v3.acl`](https://github.com/A3S-Lab/Use/blob/main/crates/extension/fixtures/manifests/plugin-v3.acl)。
