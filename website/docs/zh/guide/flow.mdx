...
A3S Use 已实现 manifest 准入、精确源码证据、签名 catalog 闭包、包生命周期顺序、reconciler owner、host-capabilities v4、manager tools v3、类型化 capability projection 与具体的 `A3sFlowLifecycleHost`。该 host 委托真实 `a3s-flow` Native TypeScript preflight，持久化精确 generation 的源码/编译产物 binding，并在 observation 时重新检查两者。未注入 `a3s-flow` host adapter 时，standalone engine 会在变更前拒绝包含必需 Flow 的包。
