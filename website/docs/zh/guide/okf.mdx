...
在 A3S Use 中，OKF 是与 Tool、MCP、Skill、UI 并列的一等认知表面，但它不是可执行 workload。

> 当前状态：M0K-B 已实现冻结的 bundle descriptor、schema-v3 manifest、递归包验证、catalog-v3 evidence、plan-v2 impact、projection receipt、Knowledge observation、capability projection、依赖闭包与 reconciler gate。A3S Knowledge 持久 stage/promote/remove 适配器和 scoped cited-retrieval 路径仍待完成；没有 promoted evidence 就不会发布该表面。
