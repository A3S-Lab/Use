...
> 当前状态：产品范围已接受 OKF；manifest contract、canonical fixture、receipt/projection 与 A3S Knowledge 原子索引协议尚未冻结。当前 schema-v3 parser 不接受未定义的 `okf` block。
...
| 表面        | 与 OKF 的区别或关系                                                                                                     |
| ----------- | ----------------------------------------------------------------------------------------------------------------------- |
| Skill       | Skill 教 Agent 如何完成任务；OKF 提供可引用的领域事实、决策和概念。目标依赖图可允许 Skill 要求一个命名 OKF generation。 |
...
- 所有 path、link、file count、expanded bytes 与 parsed-node size 都必须有界。安全 dangling link 只产生诊断；解析到包边界外的 reference 会被拒绝。
