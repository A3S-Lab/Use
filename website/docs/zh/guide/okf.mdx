...
> 当前状态：standalone lifecycle 已包含跨平台 bundled SQLite/FTS5 Knowledge 后端、完整 User/Workspace 隔离、事务化 stage/promote/remove、基于精确 projection 授权的带引用搜索、重启恢复，以及 signed install/query/upgrade/uninstall 覆盖。A3S Code 托管的 Workspace/session carrier 与生产 retention/GC 仍待完成；没有精确 promoted evidence 就不会发布该表面。
