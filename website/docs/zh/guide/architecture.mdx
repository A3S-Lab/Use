...
重放保护 immutable catalog/Index。Index 与 lease 文件都是不进入备份的派生 operational state；catalog 仍需登记到 backup/restore owner 集合。真实组合测试已经覆盖 Knowledge、Skill、catalog/Index cutover、stale admission 与
