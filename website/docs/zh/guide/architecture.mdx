...
candidate 漂移都会 fail closed。Control 还拥有独立的 clean-target publication primitive：在同一个精确
attempt 与 guard 仍被保留时，它会重新校验 semantic export、物理证据与 sidecar absence，以 no-clobber
方式原子发布，并把精确的 post-publication replay 收敛为同一个无路径结果；含糊或漂移的 candidate/live
状态会 fail closed。Complete-set activation、持久化跨 owner journal、完整 subprocess-exit 验收、生产
backup/restore 接线，以及所有消费者的协调权威切换完成前，该内核仍保持禁用。
