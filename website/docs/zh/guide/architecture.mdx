...
Control export，但 observation restore 尚未实现。这些仍是未启用的 owner primitive；Host projection、
Restore Coordinator adapter、observation restore、complete-set coordinator、完整 process-exit 验收、生产
orchestration，以及所有消费者的协调权威切换完成前，该内核仍保持禁用。
