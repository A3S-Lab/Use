...
并在到期后允许自动复用原 key。Provider timeout 必须在 claim lease 内保留固定的 observation 预算并作为 unknown evidence；取消、
...
其余 owner adapter 与 dispatcher composition 仍必须只从已提交 Control authority 与 immutable artifact
推导完整 manifest、Grant、Runtime 和 capability 输入。
