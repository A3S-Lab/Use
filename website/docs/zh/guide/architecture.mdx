...
snapshot 的精确子集，结果仍不包含路径。这些仍是未启用的 owner primitive；Host projection、Restore
Coordinator adapter、complete-set coordinator、完整 subprocess-exit 验收、生产 orchestration，以及所有
消费者的协调权威切换完成前，该内核仍保持禁用。
