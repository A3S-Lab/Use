...
字节和无路径 receipt 绑定到 Control export。空历史或仅 active 的历史不会创建 archive。这些仍是未启用的
owner primitive；Restore Coordinator 激活及 self-hosted journal reconciliation、complete-set coordinator、
完整 subprocess-exit 验收、生产 orchestration，以及所有消费者的协调权威切换完成前，该内核仍保持禁用。
