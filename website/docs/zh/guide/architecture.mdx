...
具备幂等性，不创建 lifecycle authority，并让 reference guard 跨越单独的 Control commit。Runtime、Flow、
Capability Index、invocation lease adapter 与生产 composition 仍必须只从已提交 Control authority 与
immutable artifact 推导输入。
