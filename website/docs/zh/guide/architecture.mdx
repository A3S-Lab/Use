...
未启用的 schema-v10 内核现已从 reviewed Plan 与已提交 generation 推导完整 outbox
...
与 invocation lease 现在由第三个真实的 Capability Plane adapter 共同实现。它根据已提交
candidate 与精确的终态 surface evidence 生成规范、内容寻址的 Index document，不增加第二个 SQLite 或
mutable current 文件；Control 中 applied cutover observation 是唯一 publication cursor。Admission 会在取得
...
重放保护 immutable Index。Index 与 lease 文件都是不进入备份的派生 operational state；restore 必须从已
验证 Control evidence 重建 Index。真实组合测试已经覆盖 Knowledge、Skill、cutover、stale admission 与
...
精确最终 receipt。外部或持久化 effect 后的任何歧义都保持 unknown。Runtime/Flow 生产 composition 仍必须
在重启后只从已提交 Control authority 与 immutable artifact 重建全部输入；验收使用的进程内 Runtime
selection 不是生产权威。
...
claimed 与 unknown outcome 只保留为 reconciliation evidence，三者都不能选择 desired state。
