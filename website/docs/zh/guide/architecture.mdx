...
且具有 schema、digest 与计数上限的规范 receipt 集。该证据契约不复制 payload，也不替换旧 backup
scanner。在真实 owner snapshot/verification/restore adapter、owner-port dispatch、生产 orchestration
与所有消费者的协调权威切换完成前，该内核仍保持禁用。
