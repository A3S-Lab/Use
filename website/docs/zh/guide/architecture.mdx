...
binding/selection inventory；数据库缺失时则记录零文件 manifest，且不创建 live state。其余 owner
adapter、Knowledge staged restore/activation 与 Control effect 对账、生产 orchestration，以及所有
