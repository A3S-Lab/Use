...
effect 后 checkpoint 前边界、marker 退休前的最终 checkpoint，以及删除后的进程退出都能收敛。只有完整
五 checkpoint journal 才允许 marker 缺失；乱序 live root、含糊 marker、link、重绑定和 evidence 漂移都会
fail closed。完成态 replay 只读，13 个真实子进程退出边界覆盖顶层协议。生产 backup/restore 接线、保留
attempt 的生命周期，以及所有消费者的协调权威切换完成前，该内核仍保持禁用。
