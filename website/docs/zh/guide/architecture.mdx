...
而不会回滚已可见 generation。未启用的单 effect dispatcher 现在会先提交 claim，在 owner I/O 前释放
SQLite 事务与有界 executor，再把精确 identity 路由到彼此独立的 Capability Index、invocation lease、
Runtime、Flow、Knowledge、Skill 或 UI 端口，并在之后记录 applied、rejected 或 unknown observation。
Provider timeout 必须在 claim lease 内保留固定的 observation 预算并作为 unknown evidence；取消、
可能已产生 effect 后的进程退出、过期 claim 与含糊结果都必须显式重放已提交 key。生产 lifecycle
仍不会构造它；真实 adapter 的完整 manifest、Grant、Runtime 与 capability 输入只能来自已提交
Control authority 与 immutable artifact。
私有的无路径 registry 还冻结了五个外部 payload owner ID 与精确
...
marker 绑定同一个不可变 operation，并持续阻止普通共享访问。固定 owner 顺序是 Control Store、Host
projection、Knowledge、observations、Restore Coordinator；每一步都遵循 journal、marker、owner effect、
...
snapshot、attempt、registry 与 policy，并重建或验证每个 candidate/live 边界。Journal/marker partial、五个
effect 后 checkpoint 前边界、marker 退休前的最终 checkpoint，以及 marker 删除和五个暂存树按固定顺序
退休后的进程退出都能收敛。只有完整五 checkpoint journal 才允许 marker 缺失；乱序 live root、含糊
marker、link、重绑定和 evidence 漂移都会 fail closed。完成态 replay 不再执行 owner effect，只能恢复五个
无 link 暂存树的有界固定顺序退休。18 个真实子进程退出边界覆盖顶层协议和每个退休边界。最终保留的规范
