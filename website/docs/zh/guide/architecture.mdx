...
仅在保留同一个 staged attempt 与 fence 时，重试才是幂等的。
这仍是未启用的 owner primitive；其余 owner adapter、complete-set restore coordinator、完整 process-exit
验收、生产 orchestration，以及所有消费者的协调权威切换完成前，该内核仍保持禁用。
