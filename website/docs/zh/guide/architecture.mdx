...
provider 私有标识及 package 自带诊断会被移除，或收敛为通用 code。这是对
presentation boundary 的加固，但不等于已完成 live-host reference resolution、
authentication、authorization、rate limit 或 lifecycle lease。
