...
未启用的 schema-v9 内核现已从 reviewed Plan 与已提交 generation 推导完整 outbox
...
不会成为重复 effect。Applied outcome 会持久化规范化、owner-specific 的 Capability Index、
...
publication；之后的必需 effect 失败会保留为使用相同 key 显式 reconciliation 的 pending 状态，
而不会回滚已可见 generation。私有的无路径 registry 还冻结了五个外部 payload owner ID 与精确
ACL backup policy：全局 Artifact Store 被明确排除，另外四个 owner 必须生成完整、绑定 generation、
...
缺失的 applied payload 必须存在对应 remove effect。Claimed 与 unknown outcome 只保留为 reconciliation
evidence，不能选择 desired state。离线验证通过的 Knowledge snapshot 现在可以把精确数据库暂存到
...
restore adapter 要求精确的排他 fence 与 active marker，保留该 marker 和当前 operation，只替换终态历史，
并在持久化 activation evidence 中绑定精确 before/source/target inventory。Replay 允许当前 operation 的状态
前进，但会拒绝 marker、candidate、retired record 或 publication partial 漂移。Marker-only handoff 与原生
64-record retention 都保持确定性。

私有 complete-set snapshot coordinator 现在会在同一个 maintenance fence 和 timestamp 下捕获规范 Control
export 与全部四个已注册 owner snapshot。一个无路径的规范 manifest 绑定固定 owner registry、receipt、schema、
...
Control 数据库和全部四个 owner candidate。无路径 descriptor 会先绑定 snapshot、installation、owner
registry、Knowledge policy 与固定 component set。Control 会被 checkpoint 为单一文件，按规范 export
...
candidate 漂移都会 fail closed。Control activation 现在会在修改 live state 前持久化顶层 intent。不可变
attempt 仍是 restore identity；`activation.json` 是唯一可变的有序 journal，全局
`.maintenance.restore.json` marker 绑定同一个不可变 operation，并持续阻止普通共享访问。固定写入顺序是
journal、marker、owner effect、checkpoint。Preflight 在 intent 前拒绝无效状态；写入 marker 后、原子
no-clobber 发布前，会再次校验 semantic、物理、sidecar 与 attempt 边界。Reopen 会重新取得精确 fence，绑定
相同 verified snapshot、attempt 与 policy，重建仍存在且尚未激活的 owner candidate，并收敛安全的
journal/marker temporary 与 post-publication/pre-checkpoint 边界。Effect 之后 marker 缺失或含糊、link、
重绑定和 evidence 漂移都会 fail closed。Control checkpoint 绑定规范的无路径结果，完成态 replay 不会重写
journal，全局 marker 则继续保持 active。跨 owner checkpoint 与激活顺序、marker 退休、完整
subprocess-exit 验收、生产 backup/restore 接线，以及所有消费者的协调权威切换完成前，该内核仍保持禁用。
