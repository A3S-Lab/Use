...
远端 Registry 是具名、由宿主持有的 ACL 配置。默认 `a3s` 身份不是编译进程序的 endpoint，也不是必须依赖的服务：运维方可以把它绑定到镜像或私有 Registry，在不删除信任状态的情况下停用，也可以移除 override。Release bundle 始终是独立于远端 Registry 的并列来源。
...
只有启用的来源参与发现和安装；同一个 package identity 同时出现在多个启用 Registry 时会以歧义失败。替换来源也不会改写已安装 receipt：升级继续绑定记录中的 source name、URL、TUF root、channel 与 target digest，身份漂移时 fail closed。
...
OKF 控制面门禁已经实现：缺失 Knowledge evidence 为 pending，staged 不发布，只有精确 promoted observation 才是 healthy。产生和清理这些 record 的持久 Knowledge adapter 仍在开发中。
