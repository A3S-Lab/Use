...
installer 要求 Cosign，并在下载平台归档前按精确 A3S Use tag workflow identity 与 GitHub OIDC issuer 验证 `checksums.txt`。证据缺失或无效时会 fail closed，已验证 manifest 与 bundle 会随安装版本保留。installer 脚本自身仍是独立的 bootstrap 信任边界，应先审查或通过可信系统包分发；GitHub attestation 验证仍是额外的运维检查。确定性归档序列化并不证明原生二进制可在独立干净 builder 上复现；该对比与 Release 之外的证据保留仍是 release gate。
